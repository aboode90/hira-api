#!/usr/bin/env node
/**
 * يطبّق supabase/20260819_kashier_shifts.sql على قاعدة Supabase.
 *
 * أضف إلى backend/.env:
 ...
 *
 * Usage:
 *   node scripts/apply_kashier_shifts_sql.js
 */
const fs = require('fs');
...
async function main() {
  ...
  console.log('Applying 20260819_kashier_shifts.sql ...');
  await client.connect();
  try {
    await client.query(sql);
    const { rows } = await client.query(
      "select to_regclass('public.kashier_shifts') as table_name",
    );
