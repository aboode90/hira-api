...
function parsePosConfig(profile) {
  ...
  return {
    grocery: {
      ...
      nameEn: String(config.grocery?.nameEn || config.grocery?.name_en || 'Hira Grocery').trim(),
    },
    household: {
      nameAr: String(config.household?.nameAr || config.household?.name_ar || 'طلب منزلية').trim(),
      nameEn: String(config.household?.nameEn || config.household?.name_en || 'Hira Home').trim(),
    },
  };
}
