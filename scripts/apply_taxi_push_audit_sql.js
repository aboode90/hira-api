#!/usr/bin/env node
/**
 * يطبّق supabase/20260822_taxi_push_audit_atomic.sql على قاعدة Supabase.
 *
 * أضف إلى backend/.env:
 ...
 *
 * Usage:
 *   node scripts/apply_taxi_push_audit_sql.js
 */
const fs = require('fs');
...
async function main() {
  ...
  const sqlPath = path.join(
    ...
    '..',
    'supabase',
    '20260822_taxi_push_audit_atomic.sql',
  );
  ...
  console.log('Applying 20260822_taxi_push_audit_atomic.sql ...');
  await client.connect();
  try {
    await client.query(sql);
    const { rows } = await client.query(`
      select
        to_regclass('public.taxi_push_audit') as table_name,
        to_regprocedure('public.append_taxi_request_push_audit(text, jsonb)') as fn
    `);
